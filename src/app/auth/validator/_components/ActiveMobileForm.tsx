'use client'

import {
  deleteAllCookies,
  getCookieByKey,
  setCookieByTagAndValue,
  setCookieByTagAndValueAndPath,
} from '@/actions/cookieToken'
import OTPInput from '@/components/shared/OTPinput'
import { MobileValidatorOtp, MobileValidatorRequest } from '@/services/user'
import Link from 'next/link'
import { useRouter } from 'next/navigation'
import { useCallback, useEffect, useState } from 'react'
import { Controller, SubmitHandler, useForm } from 'react-hook-form'
import toast from 'react-hot-toast'

interface InputsProps {
  otp_code: string
}

const ActiveMobileForm = ({ accessToken }: { accessToken: string }) => {
  const router = useRouter()
  const [otpButtonLoading, setOtpButtonLoading] = useState(false)
  const [timeLeft, setTimeLeft] = useState(67) // 120 seconds (2 minutes)
  const [showSendOtpButton, setShowSendOtpButton] = useState(false)
  const [otp, setOtp] = useState<string>('')

  const SendOTP = useCallback(async () => {
    const isExpireLastCode = await getCookieByKey('otpchecktimetosendnewone')
    if (!isExpireLastCode) {
      setOtpButtonLoading(true)
      const mobileValidatorReq = await MobileValidatorOtp({ accessToken })
      document.cookie =
        'otpchecktimetosendnewone=sendonotpandwaitforexpire; max-age=60;'
      setOtpButtonLoading(false)
      setTimeLeft(67)
      setShowSendOtpButton(false)

      if (!mobileValidatorReq || mobileValidatorReq.status === '-1') {
        await deleteAllCookies()
        toast.error('خطایی رخ داده، لطفا دوباره تلاش کنید.')
        setTimeout(() => {
          location.href = '/auth/login'
        }, 2000)
        return
      }

      if (mobileValidatorReq.status === '1') {
        toast.success(
          `باید شماره موبایل‌تان را تایید کنید. کد با موفقیت برای شما اس‌ام‌اس شد.`
        )
        return true
      }
    }
  }, [accessToken])

  useEffect(() => {
    SendOTP()
  }, [accessToken, SendOTP])

  useEffect(() => {
    if (timeLeft > 0) {
      const timer = setInterval(() => {
        setTimeLeft(timeLeft - 1)
      }, 1000)
      return () => clearInterval(timer)
    } else {
      setShowSendOtpButton(true)
    }
  }, [timeLeft])

  const {
    handleSubmit,
    formState: { errors, isSubmitting },
  } = useForm<InputsProps>()

  const onSubmit: SubmitHandler<InputsProps> = async () => {
    try {
      if (otp.length < 5) {
        toast.error('لطفا کد را وارد کنید')
        return
      }
      const response = await MobileValidatorRequest({
        otp_code: otp,
        accessToken: accessToken as string,
      })

      if (!response || response.status === '-1') {
        toast.error(response?.message || 'خطای پیش آمد! لطفا دوباره تلاش کنید.')
        return
      } else if (response.status === '1') {
        await setCookieByTagAndValueAndPath({
          key: 'user_status',
          value: 'ACTIVE',
          path: '/',
        })
        toast.success(
          'هویت شما تایید شد. لطفا برای ورود چند لحظه منتظر بمانید!'
        )
        location.href = `/wallet`
        return true
      }
    } catch (err: unknown) {
      console.log(err)
    }
  }

  return (
    <div className=''>
      <p className='text-zinc-600 text-center mb-3'>
        لطفا کد فعال سازی ارسال شده به موبایل تان را وارد کنید.
      </p>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div dir='ltr'>
          <OTPInput setResult={setOtp} />
        </div>
        <div className='mt-6 text-center'>
          {showSendOtpButton ? (
            <button className='text-primary' onClick={SendOTP} type='button'>
              {otpButtonLoading ? (
                <svg
                  aria-hidden='true'
                  className='w-6 h-6 text-gray-200 animate-spin fill-white'
                  viewBox='0 0 100 101'
                  fill='none'
                  xmlns='http://www.w3.org/2000/svg'>
                  <path
                    d='M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z'
                    fill='currentColor'
                  />
                  <path
                    d='M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z'
                    fill='currentFill'
                  />
                </svg>
              ) : (
                'ارسال مجدد کد'
              )}
            </button>
          ) : (
            <p className='text-amber-700'>
              {Math.floor(timeLeft / 60)}:{('0' + (timeLeft % 60)).slice(-2)}
            </p>
          )}
        </div>
        <div className='text-center mt-8'>
          <button
            className='w-full !rounded-lg fill-button h-10 flex justify-center items-center '
            type='submit'
            // disabled={isSubmitting}
          >
            {isSubmitting ? (
              <svg
                aria-hidden='true'
                className='w-6 h-6 text-gray-200 animate-spin fill-white'
                viewBox='0 0 100 101'
                fill='none'
                xmlns='http://www.w3.org/2000/svg'>
                <path
                  d='M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z'
                  fill='currentColor'
                />
                <path
                  d='M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z'
                  fill='currentFill'
                />
              </svg>
            ) : (
              'ورود به اتصال'
            )}
          </button>
        </div>
      </form>

      <div className='mt-6 sm:mt-8'>
        <p className='text-center text-zinc-600 font-light'>
          <Link
            className='ms-2 font-medium text-primary hover:underline'
            href='/auth/login'>
            ورود به حساب کاربری
          </Link>
        </p>
      </div>
    </div>
  )
}

export default ActiveMobileForm
