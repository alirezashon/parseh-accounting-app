...
// const Selectree = ({ label, theme, data = [], onUnselect }: Props) => {
...
//       {showDropdown && (
//         <div className="absolute z-50  w-full bg-[red] border border-gray-300 rounded-lg shadow-lg max-h-[400px] overflow-y-auto px-2">
//           <input
//             type="text"
...
const Selectree = ({ label, theme, data = [], onUnselect ,onSelect}: Props) => {
  const [openTrees, setOpenTrees] = useState<number[]>([])
  const [searchTerm, setSearchTerm] = useState('')
  const [searchResults, setSearchResults] = useState<TreeChartInterface[]>([])
  const [selectedNode, setSelectedNode] = useState<TreeChartInterface | null>(
  ...
  const focusNode = useCallback(
    ...
    [getAllChildIds, getParentIds, selectedNode, onUnselect]
  )
